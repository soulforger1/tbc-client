...
import { Moon, Sun, Globe, Check, LogOut, User } from "lucide-react";
import { useTheme } from "@/context/ThemeContext";
import { useAuth } from "@/context/AuthContext";
import { Card, CardContent } from "@/components/ui/card";
import { LogoutModal } from "@/components/Sidebar";
      ...
      <section>
        <div className="mb-3">
          <h2 className="text-sm font-semibold text-ink flex items-center gap-2">
            <User className="h-4 w-4 text-ink3" />
            {t("settings.account")}
          </h2>
          <p className="text-xs text-ink4 mt-0.5">{t("settings.accountSub")}</p>
        </div>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-edge px-3 py-1.5 text-xs font-medium text-ink3 transition-colors hover:border-destructive hover:text-destructive"
              >
  ...
  <button
    ...
    className={`relative flex items-center gap-3 rounded-xl border p-4 text-left transition-colors ${
      active
        ...
        : "border-edge bg-card hover:border-edge2 hover:bg-muted"
    }`}
  >
    <span className="text-2xl">{flag}</span>
    <div className="min-w-0">
      <p
        className={`text-sm font-semibold ${active ? "text-blue-500" : "text-ink"}`}
      >
        {label}
      </p>
