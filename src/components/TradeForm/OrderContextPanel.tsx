...
const Row = ({
  ...
  <div className="flex items-center justify-between gap-2 py-1.5">
    ...
    {loading ? (
      ...
      <span className={`text-xs ${bold ? "font-bold text-ink" : "text-ink2"}`}>
        {value}
      </span>
    )}
  </div>
...
export const OrderContextPanel = ({
  ...
  const total = commission != null ? tradeValue + commission + stampDuty : null;
    ...
    <div className="rounded-xl border border-edge bg-card overflow-hidden h-full flex flex-col">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-edge">
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          <span className="text-xs text-ink4 mb-0.5">
            {isMarket ? "" : displayCcy}
          </span>
        </div>
      </div>
        ...
        <div className="divide-y divide-edge">
          ...
          {!isMarket && (
            <>
              <Row
                label={t("trade.price")}
                value={(marketPrice ?? 0) > 0 ? fmtPrice(marketPrice!) : "—"}
              />
              <Row
                label={t("trade.value")}
